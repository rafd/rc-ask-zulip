...
2. A background asyncio task started in FastAPI's `lifespan` (`checkin_job.refresh_snapshot_loop`) rebuilds the snapshot every 12 hours: it calls `checkin_fetch.build_grouped(zulip_site, classify_fn=classify_cached)` and writes the result with `db.put_snapshot(...)`. On first boot (no snapshot yet) the endpoint falls back to a synchronous regex-classified build so the page isn't empty.
